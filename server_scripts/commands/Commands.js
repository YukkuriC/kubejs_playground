    ...
    // 玩家数据操作系列
    {
        ...
        // 附魔操作
        {
            ...
            let doEnchant = ctx => {
                ...
                ctx.source.server.tell(`Enchanting ${type} Lv.${level}`)
            ...
            F.then(
                ...
                cmd
                    ...
                    .executes(ctx => {
                        ...
                        if (!pool) return 0
                        ctx.source.server.tell(`Removing ${pool.length} enchantments`)
                        pool.clear()
                        return 1
                    })
                    .then(
                        cmd.argument('type', arg.STRING.create(e)).executes(ctx => {
                            ...
                            if (!pool) return 0
                            let type = arg.STRING.getResult(ctx, 'type')
                            ctx.source.server.tell(`Removing ${type} enchantments`)
            ...
            F.then(
                cmd.literal('enchant_dump').executes(ctx => {
                    ...
                    if (!pool) return 0
                    ctx.source.server.tell(`Dumping ${pool.length} enchantment(s)`)
                ...
                cmd.literal('enchant_merge').executes(ctx => {
                    ...
                    if (!poolOffhand) return ctx.source.server.tell('No enchantments in offhand'), 0
                    if (!pool) {
                    ...
                    ctx.source.server.tell(`Merging ${poolOffhand.length} enchantment(s)`)
